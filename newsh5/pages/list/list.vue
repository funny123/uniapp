				...
				uni.request({
					...
					success: (result) => {
						...
						if (result.statusCode == 200) {
							const data = result.data.result.map((news) => {
								return {
									id: news.docid,
									article_type: news.ctype,
									datetime: friendlyDate(new Date(news.date.replace(/\-/g, '/')).getTime()),
									title: news.title,
							...
							activeTab.requestParams.offset = parseInt(activeTab.requestParams.offset+20);
						}
					}
				});
			},
