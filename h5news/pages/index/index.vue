<template>
	<view class="content">
		...
		*  -->
		<!-- #ifndef MP -->
		<mix-advert ref="mixAdvert" :timedown="3" imageUrl="/static/advert.jpg" :url="advertNavUrl"></mix-advert>
		<!-- #endif -->
				...
				<swiper-item v-for="tabItem in tabBars" :key="tabItem.id">
					<scroll-view class="panel-scroll-box" :scroll-y="enableScroll" @scrolltolower="loadMore">
						...
						<view v-for="(item, index) in tabItem.newsList" :key="index" class="news-item" @click="navToDetails(item)">
							...
							<view v-if="item.hasOwnProperty('images')" :class="['img-list', 'img-list3', item.images.length === 1 && item.ctype=='news' ? 'img-list-single': '']">
								<view v-for="(imgItem, imgIndex) in item.images" :key="imgIndex" :class="['img-wrapper', 'img-wrapper1', item.images.length === 1 && item.ctype=='news' ? 'img-wrapper-single': '']">
									<image class="img" :src="imgItem"></image>
									<view class="video-tip" v-if="item.ctype=='video'">
							...
							<view :class="['bot', 'bot'+item.ctype]">
								<text class="author">{{item.author}}</text>
								<text class="time">{{item.time}}</text>
							</view>
							<view v-if="index%18==0&&index!=0">
								<InFeedAdsense data-ad-layout-key="-fb+5w+4e-db+86" data-ad-client="ca-pub-4729695073417937" data-ad-slot="2001838113">
								</InFeedAdsense>
							</view>
						</view>
		...
		computed: {
			advertNavUrl() {
				let data = {
					...
					url:encodeURIComponent('https://engine.lvehaisen.com/index/activity?appKey=3bAM2TpzSxEJq7j6zCizB4YD2q52&adslotId=294739')
				}
					...
					success: (result) => {
						if (result.statusCode == 200) {
							...
							if (type === 'add') {
								...
								tabItem.loadMoreStatus = tabItem.newsList.length > 10000 ? 2 : 0;
								// tabItem.newsList.push(item);
								tabItem.offset += 20;
							}
						}
